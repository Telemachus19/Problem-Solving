# LeetCode 300-Problem Roadmap

This is the main day-by-day roadmap for the LeetCode folder. The target is 300 easy-to-medium problems with broad DSA coverage, and the solved-history section below stays as a separate reference for items already completed in the repository.

## 600 Target Plan (Apr 18 -> Jun 30)

Start date: 18 Apr 2026
End date: 30 Jun 2026
Window: 74 days

Target: 600 solved by 30 Jun using a two-lane day plan:

- Ladder lane (ascending topics): 2-5 new problems/day.
- Reinforcement lane (speed/review): 3-4 problems/day from previously studied patterns.
- Daily total solved target: 8 problems/day, with 9 problems on 8 push days.

Math:

- Base: 74 x 8 = 592
- Push days (+1 each): 8 days x 1 = 8
- Final target by 30 Jun: 600 solved in this window

### Push Days (9 Problems)

- 20 Apr
- 27 Apr
- 04 May
- 11 May
- 18 May
- 25 May
- 08 Jun
- 22 Jun

### Ascending Topic Ladder

- 18 Apr -> 27 Apr: Arrays, strings, hashing, two pointers, sliding window, prefix sums, binary search basics.
- 28 Apr -> 10 May: Linked lists, stack/queue, monotonic stack, intervals, greedy, heap basics.
- 11 May -> 24 May: Trees/BST traversal, recursion, divide and conquer, backtracking fundamentals.
- 25 May -> 08 Jun: Graphs (BFS/DFS), topological sort, shortest paths, union-find, grid graphs.
- 09 Jun -> 20 Jun: Dynamic programming ladder (1D -> 2D -> subsequence/knapsack/state DP).
- 21 Jun -> 30 Jun: Mixed review, weak-topic repair, design/implementation, timed mocks.

### Weekly Execution Template

Weekly template for 18 Apr to 30 Jun:

- Default day: 4 ladder + 4 reinforcement
- Push day: 4 ladder + 5 reinforcement
- Low-energy day fallback: 2-3 ladder + remaining reinforcement

This keeps learning in ascending order while still building enough solve volume for the 600 target.

### Date Milestones

- 30 Apr target cumulative solved in window: 104+
- 31 May target cumulative solved in window: 352+
- 30 Jun target cumulative solved in window: 600

## Weekly Milestones (Apr 18 -> Jun 30)

- Week 1 (18 Apr -> 24 Apr): Arrays, strings, hashing, two pointers, sliding window.
- Week 2 (25 Apr -> 01 May): Prefix sums, binary search, linked lists, stack/queue basics.
- Week 3 (02 May -> 08 May): Monotonic stack, intervals, greedy, heap basics.
- Week 4 (09 May -> 15 May): Trees/BST traversals and recursion foundations.
- Week 5 (16 May -> 22 May): Tree construction, divide and conquer, backtracking I.
- Week 6 (23 May -> 29 May): Backtracking II, graph traversals (BFS/DFS), topological sort.
- Week 7 (30 May -> 05 Jun): Union-find, shortest paths, grid graphs.
- Week 8 (06 Jun -> 12 Jun): Dynamic programming 1D and classic transition patterns.
- Week 9 (13 Jun -> 19 Jun): Dynamic programming 2D, subsequence, knapsack-style states.
- Week 10 (20 Jun -> 26 Jun): Mixed repair: weak topics, graph/DP reinforcement, design practice.
- Week 11 (27 Jun -> 30 Jun): Final revision and mock-style mixed sets.

### Daily Ladder Problem List (New Problems)

- Start day: 18 Apr 2026.
- New-problem lane: 4/day across all scheduled dates below.
- Keep the reinforcement lane (3-4 review solves/day) from the 600 plan above to stay on the 600-solved trajectory.

#### 18 Apr (Day 01)

- [x] [LC-26-remove-duplicates-from-sorted-array](https://leetcode.com/problems/remove-duplicates-from-sorted-array)
- [ ] [LC-88-merge-sorted-array](https://leetcode.com/problems/merge-sorted-array)
- [ ] [LC-169-majority-element](https://leetcode.com/problems/majority-element)
- [ ] [LC-189-rotate-array](https://leetcode.com/problems/rotate-array)

#### 19 Apr (Day 02)

- [ ] [LC-414-third-maximum-number](https://leetcode.com/problems/third-maximum-number)
- [ ] [LC-448-find-all-numbers-disappeared-in-an-array](https://leetcode.com/problems/find-all-numbers-disappeared-in-an-array)
- [ ] [LC-566-reshape-the-matrix](https://leetcode.com/problems/reshape-the-matrix)
- [ ] [LC-724-find-pivot-index](https://leetcode.com/problems/find-pivot-index)

#### 20 Apr (Day 03)

- [ ] [LC-905-sort-array-by-parity](https://leetcode.com/problems/sort-array-by-parity)
- [ ] [LC-125-valid-palindrome](https://leetcode.com/problems/valid-palindrome)
- [ ] [LC-344-reverse-string](https://leetcode.com/problems/reverse-string)
- [ ] [LC-345-reverse-vowels-of-a-string](https://leetcode.com/problems/reverse-vowels-of-a-string)

#### 21 Apr (Day 04)

- [ ] [LC-680-valid-palindrome-ii](https://leetcode.com/problems/valid-palindrome-ii)
- [ ] [LC-392-is-subsequence](https://leetcode.com/problems/is-subsequence)
- [ ] [LC-28-find-the-index-of-the-first-occurrence-in-a-string](https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string)
- [ ] [LC-415-add-strings](https://leetcode.com/problems/add-strings)

#### 22 Apr (Day 05)

- [ ] [LC-58-length-of-last-word](https://leetcode.com/problems/length-of-last-word)
- [ ] [LC-557-reverse-words-in-a-string-iii](https://leetcode.com/problems/reverse-words-in-a-string-iii)
- [ ] [LC-796-rotate-string](https://leetcode.com/problems/rotate-string)
- [ ] [LC-36-valid-sudoku](https://leetcode.com/problems/valid-sudoku)

#### 23 Apr (Day 06)

- [ ] [LC-49-group-anagrams](https://leetcode.com/problems/group-anagrams)
- [ ] [LC-128-longest-consecutive-sequence](https://leetcode.com/problems/longest-consecutive-sequence)
- [ ] [LC-205-isomorphic-strings](https://leetcode.com/problems/isomorphic-strings)
- [ ] [LC-290-word-pattern](https://leetcode.com/problems/word-pattern)

#### 24 Apr (Day 07)

- [ ] [LC-387-first-unique-character-in-a-string](https://leetcode.com/problems/first-unique-character-in-a-string)
- [ ] [LC-219-contains-duplicate-ii](https://leetcode.com/problems/contains-duplicate-ii)
- [ ] [LC-451-sort-characters-by-frequency](https://leetcode.com/problems/sort-characters-by-frequency)
- [ ] [LC-1207-unique-number-of-occurrences](https://leetcode.com/problems/unique-number-of-occurrences)

#### 25 Apr (Day 08)

- [ ] [LC-347-top-k-frequent-elements](https://leetcode.com/problems/top-k-frequent-elements)
- [ ] [LC-3-longest-substring-without-repeating-characters](https://leetcode.com/problems/longest-substring-without-repeating-characters)
- [ ] [LC-209-minimum-size-subarray-sum](https://leetcode.com/problems/minimum-size-subarray-sum)
- [ ] [LC-239-sliding-window-maximum](https://leetcode.com/problems/sliding-window-maximum)

#### 26 Apr (Day 09)

- [ ] [LC-567-permutation-in-string](https://leetcode.com/problems/permutation-in-string)
- [ ] [LC-643-maximum-average-subarray-i](https://leetcode.com/problems/maximum-average-subarray-i)
- [ ] [LC-930-binary-subarrays-with-sum](https://leetcode.com/problems/binary-subarrays-with-sum)
- [ ] [LC-560-subarray-sum-equals-k](https://leetcode.com/problems/subarray-sum-equals-k)

#### 27 Apr (Day 10)

- [ ] [LC-974-subarray-sums-divisible-by-k](https://leetcode.com/problems/subarray-sums-divisible-by-k)
- [ ] [LC-713-subarray-product-less-than-k](https://leetcode.com/problems/subarray-product-less-than-k)
- [ ] [LC-424-longest-repeating-character-replacement](https://leetcode.com/problems/longest-repeating-character-replacement)
- [ ] [LC-206-reverse-linked-list](https://leetcode.com/problems/reverse-linked-list)

#### 28 Apr (Day 11)

- [ ] [LC-83-remove-duplicates-from-sorted-list](https://leetcode.com/problems/remove-duplicates-from-sorted-list)
- [ ] [LC-141-linked-list-cycle](https://leetcode.com/problems/linked-list-cycle)
- [ ] [LC-142-linked-list-cycle-ii](https://leetcode.com/problems/linked-list-cycle-ii)
- [ ] [LC-19-remove-nth-node-from-end-of-list](https://leetcode.com/problems/remove-nth-node-from-end-of-list)

#### 29 Apr (Day 12)

- [ ] [LC-160-intersection-of-two-linked-lists](https://leetcode.com/problems/intersection-of-two-linked-lists)
- [ ] [LC-234-palindrome-linked-list](https://leetcode.com/problems/palindrome-linked-list)
- [ ] [LC-86-partition-list](https://leetcode.com/problems/partition-list)
- [ ] [LC-61-rotate-list](https://leetcode.com/problems/rotate-list)

#### 30 Apr (Day 13)

- [ ] [LC-92-reverse-linked-list-ii](https://leetcode.com/problems/reverse-linked-list-ii)
- [ ] [LC-150-evaluate-reverse-polish-notation](https://leetcode.com/problems/evaluate-reverse-polish-notation)
- [ ] [LC-71-simplify-path](https://leetcode.com/problems/simplify-path)
- [ ] [LC-232-implement-queue-using-stacks](https://leetcode.com/problems/implement-queue-using-stacks)

#### 01 May (Day 14)

- [ ] [LC-225-implement-stack-using-queues](https://leetcode.com/problems/implement-stack-using-queues)
- [ ] [LC-496-next-greater-element-i](https://leetcode.com/problems/next-greater-element-i)
- [ ] [LC-503-next-greater-element-ii](https://leetcode.com/problems/next-greater-element-ii)
- [ ] [LC-739-daily-temperatures](https://leetcode.com/problems/daily-temperatures)

#### 02 May (Day 15)

- [ ] [LC-901-online-stock-span](https://leetcode.com/problems/online-stock-span)
- [ ] [LC-622-design-circular-queue](https://leetcode.com/problems/design-circular-queue)
- [ ] [LC-933-number-of-recent-calls](https://leetcode.com/problems/number-of-recent-calls)
- [ ] [LC-704-binary-search](https://leetcode.com/problems/binary-search)

#### 03 May (Day 16)

- [ ] [LC-34-find-first-and-last-position-of-element-in-sorted-array](https://leetcode.com/problems/find-first-and-last-position-of-element-in-sorted-array)
- [ ] [LC-33-search-in-rotated-sorted-array](https://leetcode.com/problems/search-in-rotated-sorted-array)
- [ ] [LC-74-search-a-2d-matrix](https://leetcode.com/problems/search-a-2d-matrix)
- [ ] [LC-153-find-minimum-in-rotated-sorted-array](https://leetcode.com/problems/find-minimum-in-rotated-sorted-array)

#### 04 May (Day 17)

- [ ] [LC-162-find-peak-element](https://leetcode.com/problems/find-peak-element)
- [ ] [LC-69-sqrtx](https://leetcode.com/problems/sqrtx)
- [ ] [LC-278-first-bad-version](https://leetcode.com/problems/first-bad-version)
- [ ] [LC-875-koko-eating-bananas](https://leetcode.com/problems/koko-eating-bananas)

#### 05 May (Day 18)

- [ ] [LC-658-find-k-closest-elements](https://leetcode.com/problems/find-k-closest-elements)
- [ ] [LC-56-merge-intervals](https://leetcode.com/problems/merge-intervals)
- [ ] [LC-57-insert-interval](https://leetcode.com/problems/insert-interval)
- [ ] [LC-435-non-overlapping-intervals](https://leetcode.com/problems/non-overlapping-intervals)

#### 06 May (Day 19)

- [ ] [LC-452-minimum-number-of-arrows-to-burst-balloons](https://leetcode.com/problems/minimum-number-of-arrows-to-burst-balloons)
- [ ] [LC-122-best-time-to-buy-and-sell-stock-ii](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-ii)
- [ ] [LC-55-jump-game](https://leetcode.com/problems/jump-game)
- [ ] [LC-45-jump-game-ii](https://leetcode.com/problems/jump-game-ii)

#### 07 May (Day 20)

- [ ] [LC-134-gas-station](https://leetcode.com/problems/gas-station)
- [ ] [LC-763-partition-labels](https://leetcode.com/problems/partition-labels)
- [ ] [LC-406-queue-reconstruction-by-height](https://leetcode.com/problems/queue-reconstruction-by-height)
- [ ] [LC-94-binary-tree-inorder-traversal](https://leetcode.com/problems/binary-tree-inorder-traversal)

#### 08 May (Day 21)

- [ ] [LC-144-binary-tree-preorder-traversal](https://leetcode.com/problems/binary-tree-preorder-traversal)
- [ ] [LC-145-binary-tree-postorder-traversal](https://leetcode.com/problems/binary-tree-postorder-traversal)
- [ ] [LC-102-binary-tree-level-order-traversal](https://leetcode.com/problems/binary-tree-level-order-traversal)
- [ ] [LC-101-symmetric-tree](https://leetcode.com/problems/symmetric-tree)

#### 09 May (Day 22)

- [ ] [LC-100-same-tree](https://leetcode.com/problems/same-tree)
- [ ] [LC-110-balanced-binary-tree](https://leetcode.com/problems/balanced-binary-tree)
- [ ] [LC-111-minimum-depth-of-binary-tree](https://leetcode.com/problems/minimum-depth-of-binary-tree)
- [ ] [LC-112-path-sum](https://leetcode.com/problems/path-sum)

#### 10 May (Day 23)

- [ ] [LC-257-binary-tree-paths](https://leetcode.com/problems/binary-tree-paths)
- [ ] [LC-700-search-in-a-binary-search-tree](https://leetcode.com/problems/search-in-a-binary-search-tree)
- [ ] [LC-701-insert-into-a-binary-search-tree](https://leetcode.com/problems/insert-into-a-binary-search-tree)
- [ ] [LC-98-validate-binary-search-tree](https://leetcode.com/problems/validate-binary-search-tree)

#### 11 May (Day 24)

- [ ] [LC-235-lowest-common-ancestor-of-a-binary-search-tree](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree)
- [ ] [LC-450-delete-node-in-a-bst](https://leetcode.com/problems/delete-node-in-a-bst)
- [ ] [LC-530-minimum-absolute-difference-in-bst](https://leetcode.com/problems/minimum-absolute-difference-in-bst)
- [ ] [LC-108-convert-sorted-array-to-binary-search-tree](https://leetcode.com/problems/convert-sorted-array-to-binary-search-tree)

#### 12 May (Day 25)

- [ ] [LC-109-convert-sorted-list-to-binary-search-tree](https://leetcode.com/problems/convert-sorted-list-to-binary-search-tree)
- [ ] [LC-938-range-sum-of-bst](https://leetcode.com/problems/range-sum-of-bst)
- [ ] [LC-173-binary-search-tree-iterator](https://leetcode.com/problems/binary-search-tree-iterator)
- [ ] [LC-1046-last-stone-weight](https://leetcode.com/problems/last-stone-weight)

#### 13 May (Day 26)

- [ ] [LC-215-kth-largest-element-in-an-array](https://leetcode.com/problems/kth-largest-element-in-an-array)
- [ ] [LC-692-top-k-frequent-words](https://leetcode.com/problems/top-k-frequent-words)
- [ ] [LC-973-k-closest-points-to-origin](https://leetcode.com/problems/k-closest-points-to-origin)
- [ ] [LC-373-find-k-pairs-with-smallest-sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums)

#### 14 May (Day 27)

- [ ] [LC-378-kth-smallest-element-in-a-sorted-matrix](https://leetcode.com/problems/kth-smallest-element-in-a-sorted-matrix)
- [ ] [LC-264-ugly-number-ii](https://leetcode.com/problems/ugly-number-ii)
- [ ] [LC-767-reorganize-string](https://leetcode.com/problems/reorganize-string)
- [ ] [LC-621-task-scheduler](https://leetcode.com/problems/task-scheduler)

#### 15 May (Day 28)

- [ ] [LC-502-ipo](https://leetcode.com/problems/ipo)
- [ ] [LC-17-letter-combinations-of-a-phone-number](https://leetcode.com/problems/letter-combinations-of-a-phone-number)
- [ ] [LC-22-generate-parentheses](https://leetcode.com/problems/generate-parentheses)
- [ ] [LC-39-combination-sum](https://leetcode.com/problems/combination-sum)

#### 16 May (Day 29)

- [ ] [LC-40-combination-sum-ii](https://leetcode.com/problems/combination-sum-ii)
- [ ] [LC-46-permutations](https://leetcode.com/problems/permutations)
- [ ] [LC-47-permutations-ii](https://leetcode.com/problems/permutations-ii)
- [ ] [LC-77-combinations](https://leetcode.com/problems/combinations)

#### 17 May (Day 30)

- [ ] [LC-78-subsets](https://leetcode.com/problems/subsets)
- [ ] [LC-90-subsets-ii](https://leetcode.com/problems/subsets-ii)
- [ ] [LC-131-palindrome-partitioning](https://leetcode.com/problems/palindrome-partitioning)
- [ ] [LC-93-restore-ip-addresses](https://leetcode.com/problems/restore-ip-addresses)

#### 18 May (Day 31)

- [ ] [LC-79-word-search](https://leetcode.com/problems/word-search)
- [ ] [LC-216-combination-sum-iii](https://leetcode.com/problems/combination-sum-iii)
- [ ] [LC-491-increasing-subsequences](https://leetcode.com/problems/increasing-subsequences)
- [ ] [LC-784-letter-case-permutation](https://leetcode.com/problems/letter-case-permutation)

#### 19 May (Day 32)

- [ ] [LC-306-additive-number](https://leetcode.com/problems/additive-number)
- [ ] [LC-254-factor-combinations](https://leetcode.com/problems/factor-combinations)
- [ ] [LC-386-lexicographical-numbers](https://leetcode.com/problems/lexicographical-numbers)
- [ ] [LC-1079-letter-tile-possibilities](https://leetcode.com/problems/letter-tile-possibilities)

#### 20 May (Day 33)

- [ ] [LC-89-gray-code](https://leetcode.com/problems/gray-code)
- [ ] [LC-200-number-of-islands](https://leetcode.com/problems/number-of-islands)
- [ ] [LC-695-max-area-of-island](https://leetcode.com/problems/max-area-of-island)
- [ ] [LC-733-flood-fill](https://leetcode.com/problems/flood-fill)

#### 21 May (Day 34)

- [ ] [LC-994-rotting-oranges](https://leetcode.com/problems/rotting-oranges)
- [ ] [LC-130-surrounded-regions](https://leetcode.com/problems/surrounded-regions)
- [ ] [LC-133-clone-graph](https://leetcode.com/problems/clone-graph)
- [ ] [LC-207-course-schedule](https://leetcode.com/problems/course-schedule)

#### 22 May (Day 35)

- [ ] [LC-210-course-schedule-ii](https://leetcode.com/problems/course-schedule-ii)
- [ ] [LC-547-number-of-provinces](https://leetcode.com/problems/number-of-provinces)
- [ ] [LC-785-is-graph-bipartite](https://leetcode.com/problems/is-graph-bipartite)
- [ ] [LC-261-graph-valid-tree](https://leetcode.com/problems/graph-valid-tree)

#### 23 May (Day 36)

- [ ] [LC-323-number-of-connected-components-in-an-undirected-graph](https://leetcode.com/problems/number-of-connected-components-in-an-undirected-graph)
- [ ] [LC-399-evaluate-division](https://leetcode.com/problems/evaluate-division)
- [ ] [LC-684-redundant-connection](https://leetcode.com/problems/redundant-connection)
- [ ] [LC-721-accounts-merge](https://leetcode.com/problems/accounts-merge)

#### 24 May (Day 37)

- [ ] [LC-947-most-stones-removed-with-same-row-or-column](https://leetcode.com/problems/most-stones-removed-with-same-row-or-column)
- [ ] [LC-990-satisfiability-of-equality-equations](https://leetcode.com/problems/satisfiability-of-equality-equations)
- [ ] [LC-1061-lexicographically-smallest-equivalent-string](https://leetcode.com/problems/lexicographically-smallest-equivalent-string)
- [ ] [LC-1319-number-of-operations-to-make-network-connected](https://leetcode.com/problems/number-of-operations-to-make-network-connected)

#### 25 May (Day 38)

- [ ] [LC-1135-connecting-cities-with-minimum-cost](https://leetcode.com/problems/connecting-cities-with-minimum-cost)
- [ ] [LC-136-single-number](https://leetcode.com/problems/single-number)
- [ ] [LC-137-single-number-ii](https://leetcode.com/problems/single-number-ii)
- [ ] [LC-190-reverse-bits](https://leetcode.com/problems/reverse-bits)

#### 26 May (Day 39)

- [ ] [LC-191-number-of-1-bits](https://leetcode.com/problems/number-of-1-bits)
- [ ] [LC-231-power-of-two](https://leetcode.com/problems/power-of-two)
- [ ] [LC-268-missing-number](https://leetcode.com/problems/missing-number)
- [ ] [LC-201-bitwise-and-of-numbers-range](https://leetcode.com/problems/bitwise-and-of-numbers-range)

#### 27 May (Day 40)

- [ ] [LC-338-counting-bits](https://leetcode.com/problems/counting-bits)
- [ ] [LC-461-hamming-distance](https://leetcode.com/problems/hamming-distance)
- [ ] [LC-476-number-complement](https://leetcode.com/problems/number-complement)
- [ ] [LC-70-climbing-stairs](https://leetcode.com/problems/climbing-stairs)

#### 28 May (Day 41)

- [ ] [LC-198-house-robber](https://leetcode.com/problems/house-robber)
- [ ] [LC-213-house-robber-ii](https://leetcode.com/problems/house-robber-ii)
- [ ] [LC-509-fibonacci-number](https://leetcode.com/problems/fibonacci-number)
- [ ] [LC-746-min-cost-climbing-stairs](https://leetcode.com/problems/min-cost-climbing-stairs)

#### 29 May (Day 42)

- [ ] [LC-62-unique-paths](https://leetcode.com/problems/unique-paths)
- [ ] [LC-63-unique-paths-ii](https://leetcode.com/problems/unique-paths-ii)
- [ ] [LC-64-minimum-path-sum](https://leetcode.com/problems/minimum-path-sum)
- [ ] [LC-120-triangle](https://leetcode.com/problems/triangle)

#### 30 May (Day 43)

- [ ] [LC-91-decode-ways](https://leetcode.com/problems/decode-ways)
- [ ] [LC-300-longest-increasing-subsequence](https://leetcode.com/problems/longest-increasing-subsequence)
- [ ] [LC-53-maximum-subarray](https://leetcode.com/problems/maximum-subarray)
- [ ] [LC-152-maximum-product-subarray](https://leetcode.com/problems/maximum-product-subarray)

#### 31 May (Day 44)

- [ ] [LC-139-word-break](https://leetcode.com/problems/word-break)
- [ ] [LC-416-partition-equal-subset-sum](https://leetcode.com/problems/partition-equal-subset-sum)
- [ ] [LC-494-target-sum](https://leetcode.com/problems/target-sum)
- [ ] [LC-377-combination-sum-iv](https://leetcode.com/problems/combination-sum-iv)

#### 01 Jun (Day 45)

- [ ] [LC-647-palindromic-substrings](https://leetcode.com/problems/palindromic-substrings)
- [ ] [LC-5-longest-palindromic-substring](https://leetcode.com/problems/longest-palindromic-substring)
- [ ] [LC-673-number-of-longest-increasing-subsequence](https://leetcode.com/problems/number-of-longest-increasing-subsequence)
- [ ] [LC-1143-longest-common-subsequence](https://leetcode.com/problems/longest-common-subsequence)

#### 02 Jun (Day 46)

- [ ] [LC-583-delete-operation-for-two-strings](https://leetcode.com/problems/delete-operation-for-two-strings)
- [ ] [LC-97-interleaving-string](https://leetcode.com/problems/interleaving-string)
- [ ] [LC-516-longest-palindromic-subsequence](https://leetcode.com/problems/longest-palindromic-subsequence)
- [ ] [LC-221-maximal-square](https://leetcode.com/problems/maximal-square)

#### 03 Jun (Day 47)

- [ ] [LC-279-perfect-squares](https://leetcode.com/problems/perfect-squares)
- [ ] [LC-322-coin-change](https://leetcode.com/problems/coin-change)
- [ ] [LC-518-coin-change-ii](https://leetcode.com/problems/coin-change-ii)
- [ ] [LC-368-largest-divisible-subset](https://leetcode.com/problems/largest-divisible-subset)

#### 04 Jun (Day 48)

- [ ] [LC-1035-uncrossed-lines](https://leetcode.com/problems/uncrossed-lines)
- [ ] [LC-103-binary-tree-zigzag-level-order-traversal](https://leetcode.com/problems/binary-tree-zigzag-level-order-traversal)
- [ ] [LC-199-binary-tree-right-side-view](https://leetcode.com/problems/binary-tree-right-side-view)
- [ ] [LC-515-find-largest-value-in-each-tree-row](https://leetcode.com/problems/find-largest-value-in-each-tree-row)

#### 05 Jun (Day 49)

- [ ] [LC-637-average-of-levels-in-binary-tree](https://leetcode.com/problems/average-of-levels-in-binary-tree)
- [ ] [LC-222-count-complete-tree-nodes](https://leetcode.com/problems/count-complete-tree-nodes)
- [ ] [LC-226-invert-binary-tree](https://leetcode.com/problems/invert-binary-tree)
- [ ] [LC-230-kth-smallest-element-in-a-bst](https://leetcode.com/problems/kth-smallest-element-in-a-bst)

#### 06 Jun (Day 50)

- [ ] [LC-236-lowest-common-ancestor-of-a-binary-tree](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-tree)
- [ ] [LC-572-subtree-of-another-tree](https://leetcode.com/problems/subtree-of-another-tree)
- [ ] [LC-654-maximum-binary-tree](https://leetcode.com/problems/maximum-binary-tree)
- [ ] [LC-105-construct-binary-tree-from-preorder-and-inorder-traversal](https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal)

#### 07 Jun (Day 51)

- [ ] [LC-106-construct-binary-tree-from-inorder-and-postorder-traversal](https://leetcode.com/problems/construct-binary-tree-from-inorder-and-postorder-traversal)
- [ ] [LC-114-flatten-binary-tree-to-linked-list](https://leetcode.com/problems/flatten-binary-tree-to-linked-list)
- [ ] [LC-116-populating-next-right-pointers-in-each-node](https://leetcode.com/problems/populating-next-right-pointers-in-each-node)
- [ ] [LC-117-populating-next-right-pointers-in-each-node-ii](https://leetcode.com/problems/populating-next-right-pointers-in-each-node-ii)

#### 08 Jun (Day 52)

- [ ] [LC-129-sum-root-to-leaf-numbers](https://leetcode.com/problems/sum-root-to-leaf-numbers)
- [ ] [LC-337-house-robber-iii](https://leetcode.com/problems/house-robber-iii)
- [ ] [LC-437-path-sum-iii](https://leetcode.com/problems/path-sum-iii)
- [ ] [LC-623-add-one-row-to-tree](https://leetcode.com/problems/add-one-row-to-tree)

#### 09 Jun (Day 53)

- [ ] [LC-653-two-sum-iv-input-is-a-bst](https://leetcode.com/problems/two-sum-iv-input-is-a-bst)
- [ ] [LC-1046-last-stone-weight](https://leetcode.com/problems/last-stone-weight)
- [ ] [LC-215-kth-largest-element-in-an-array](https://leetcode.com/problems/kth-largest-element-in-an-array)
- [ ] [LC-692-top-k-frequent-words](https://leetcode.com/problems/top-k-frequent-words)

#### 10 Jun (Day 54)

- [ ] [LC-973-k-closest-points-to-origin](https://leetcode.com/problems/k-closest-points-to-origin)
- [ ] [LC-373-find-k-pairs-with-smallest-sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums)
- [ ] [LC-378-kth-smallest-element-in-a-sorted-matrix](https://leetcode.com/problems/kth-smallest-element-in-a-sorted-matrix)
- [ ] [LC-264-ugly-number-ii](https://leetcode.com/problems/ugly-number-ii)

#### 11 Jun (Day 55)

- [ ] [LC-767-reorganize-string](https://leetcode.com/problems/reorganize-string)
- [ ] [LC-621-task-scheduler](https://leetcode.com/problems/task-scheduler)
- [ ] [LC-502-ipo](https://leetcode.com/problems/ipo)
- [ ] [LC-54-spiral-matrix](https://leetcode.com/problems/spiral-matrix)

#### 12 Jun (Day 56)

- [ ] [LC-59-spiral-matrix-ii](https://leetcode.com/problems/spiral-matrix-ii)
- [ ] [LC-73-set-matrix-zeroes](https://leetcode.com/problems/set-matrix-zeroes)
- [ ] [LC-48-rotate-image](https://leetcode.com/problems/rotate-image)
- [ ] [LC-867-transpose-matrix](https://leetcode.com/problems/transpose-matrix)

#### 13 Jun (Day 57)

- [ ] [LC-498-diagonal-traverse](https://leetcode.com/problems/diagonal-traverse)
- [ ] [LC-1572-matrix-diagonal-sum](https://leetcode.com/problems/matrix-diagonal-sum)
- [ ] [LC-240-search-a-2d-matrix-ii](https://leetcode.com/problems/search-a-2d-matrix-ii)
- [ ] [LC-885-spiral-matrix-iii](https://leetcode.com/problems/spiral-matrix-iii)

#### 14 Jun (Day 58)

- [ ] [LC-1260-shift-2d-grid](https://leetcode.com/problems/shift-2d-grid)
- [ ] [LC-151-reverse-words-in-a-string](https://leetcode.com/problems/reverse-words-in-a-string)
- [ ] [LC-186-reverse-words-in-a-string-ii](https://leetcode.com/problems/reverse-words-in-a-string-ii)
- [ ] [LC-168-excel-sheet-column-title](https://leetcode.com/problems/excel-sheet-column-title)

#### 15 Jun (Day 59)

- [ ] [LC-171-excel-sheet-column-number](https://leetcode.com/problems/excel-sheet-column-number)
- [ ] [LC-443-string-compression](https://leetcode.com/problems/string-compression)
- [ ] [LC-541-reverse-string-ii](https://leetcode.com/problems/reverse-string-ii)
- [ ] [LC-844-backspace-string-compare](https://leetcode.com/problems/backspace-string-compare)

#### 16 Jun (Day 60)

- [ ] [LC-394-decode-string](https://leetcode.com/problems/decode-string)
- [ ] [LC-227-basic-calculator-ii](https://leetcode.com/problems/basic-calculator-ii)
- [ ] [LC-921-minimum-add-to-make-parentheses-valid](https://leetcode.com/problems/minimum-add-to-make-parentheses-valid)
- [ ] [LC-303-range-sum-query-immutable](https://leetcode.com/problems/range-sum-query-immutable)

#### 17 Jun (Day 61)

- [ ] [LC-304-range-sum-query-2d-immutable](https://leetcode.com/problems/range-sum-query-2d-immutable)
- [ ] [LC-413-arithmetic-slices](https://leetcode.com/problems/arithmetic-slices)
- [ ] [LC-525-contiguous-array](https://leetcode.com/problems/contiguous-array)
- [ ] [LC-985-sum-of-even-numbers-after-queries](https://leetcode.com/problems/sum-of-even-numbers-after-queries)

#### 18 Jun (Day 62)

- [ ] [LC-1109-corporate-flight-bookings](https://leetcode.com/problems/corporate-flight-bookings)
- [ ] [LC-1094-car-pooling](https://leetcode.com/problems/car-pooling)
- [ ] [LC-1310-xor-queries-of-a-subarray](https://leetcode.com/problems/xor-queries-of-a-subarray)
- [ ] [LC-1480-running-sum-of-1d-array](https://leetcode.com/problems/running-sum-of-1d-array)

#### 19 Jun (Day 63)

- [ ] [LC-1590-make-sum-divisible-by-p](https://leetcode.com/problems/make-sum-divisible-by-p)
- [ ] [LC-41-first-missing-positive](https://leetcode.com/problems/first-missing-positive)
- [ ] [LC-66-plus-one](https://leetcode.com/problems/plus-one)
- [ ] [LC-67-add-binary](https://leetcode.com/problems/add-binary)

#### 20 Jun (Day 64)

- [ ] [LC-75-sort-colors](https://leetcode.com/problems/sort-colors)
- [ ] [LC-80-remove-duplicates-from-sorted-array-ii](https://leetcode.com/problems/remove-duplicates-from-sorted-array-ii)
- [ ] [LC-179-largest-number](https://leetcode.com/problems/largest-number)
- [ ] [LC-238-product-of-array-except-self](https://leetcode.com/problems/product-of-array-except-self)

#### 21 Jun (Day 65)

- [ ] [LC-324-wiggle-sort-ii](https://leetcode.com/problems/wiggle-sort-ii)
- [ ] [LC-769-max-chunks-to-make-sorted](https://leetcode.com/problems/max-chunks-to-make-sorted)
- [ ] [LC-977-squares-of-a-sorted-array](https://leetcode.com/problems/squares-of-a-sorted-array)
- [ ] [LC-2-add-two-numbers](https://leetcode.com/problems/add-two-numbers)

#### 22 Jun (Day 66)

- [ ] [LC-24-swap-nodes-in-pairs](https://leetcode.com/problems/swap-nodes-in-pairs)
- [ ] [LC-25-reverse-nodes-in-k-group](https://leetcode.com/problems/reverse-nodes-in-k-group)
- [ ] [LC-82-remove-duplicates-from-sorted-list-ii](https://leetcode.com/problems/remove-duplicates-from-sorted-list-ii)
- [ ] [LC-138-copy-list-with-random-pointer](https://leetcode.com/problems/copy-list-with-random-pointer)

#### 23 Jun (Day 67)

- [ ] [LC-143-reorder-list](https://leetcode.com/problems/reorder-list)
- [ ] [LC-148-sort-list](https://leetcode.com/problems/sort-list)
- [ ] [LC-203-remove-linked-list-elements](https://leetcode.com/problems/remove-linked-list-elements)
- [ ] [LC-328-odd-even-linked-list](https://leetcode.com/problems/odd-even-linked-list)

#### 24 Jun (Day 68)

- [ ] [LC-445-add-two-numbers-ii](https://leetcode.com/problems/add-two-numbers-ii)
- [ ] [LC-743-network-delay-time](https://leetcode.com/problems/network-delay-time)
- [ ] [LC-787-cheapest-flights-within-k-stops](https://leetcode.com/problems/cheapest-flights-within-k-stops)
- [ ] [LC-797-all-paths-from-source-to-target](https://leetcode.com/problems/all-paths-from-source-to-target)

#### 25 Jun (Day 69)

- [ ] [LC-802-find-eventual-safe-states](https://leetcode.com/problems/find-eventual-safe-states)
- [ ] [LC-841-keys-and-rooms](https://leetcode.com/problems/keys-and-rooms)
- [ ] [LC-886-possible-bipartition](https://leetcode.com/problems/possible-bipartition)
- [ ] [LC-1091-shortest-path-in-binary-matrix](https://leetcode.com/problems/shortest-path-in-binary-matrix)

#### 26 Jun (Day 70)

- [ ] [LC-1197-minimum-knight-moves](https://leetcode.com/problems/minimum-knight-moves)
- [ ] [LC-1334-find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance](https://leetcode.com/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance)
- [ ] [LC-1514-path-with-maximum-probability](https://leetcode.com/problems/path-with-maximum-probability)
- [ ] [LC-208-implement-trie-prefix-tree](https://leetcode.com/problems/implement-trie-prefix-tree)

#### 27 Jun (Day 71)

- [ ] [LC-211-design-add-and-search-words-data-structure](https://leetcode.com/problems/design-add-and-search-words-data-structure)
- [ ] [LC-380-insert-delete-getrandom-o1](https://leetcode.com/problems/insert-delete-getrandom-o1)
- [ ] [LC-705-design-hashset](https://leetcode.com/problems/design-hashset)
- [ ] [LC-706-design-hashmap](https://leetcode.com/problems/design-hashmap)

#### 28 Jun (Day 72)

- [ ] [LC-146-lru-cache](https://leetcode.com/problems/lru-cache)
- [ ] [LC-981-time-based-key-value-store](https://leetcode.com/problems/time-based-key-value-store)
- [ ] [LC-348-design-tic-tac-toe](https://leetcode.com/problems/design-tic-tac-toe)
- [ ] [LC-729-my-calendar-i](https://leetcode.com/problems/my-calendar-i)

#### 29 Jun (Day 73)

- [ ] [LC-641-design-circular-deque](https://leetcode.com/problems/design-circular-deque)
- [ ] [LC-31-next-permutation](https://leetcode.com/problems/next-permutation)
- [ ] [LC-43-multiply-strings](https://leetcode.com/problems/multiply-strings)
- [ ] [LC-50-powx-n](https://leetcode.com/problems/powx-n)

#### 30 Jun (Day 74)

- [ ] [LC-118-pascals-triangle](https://leetcode.com/problems/pascals-triangle)
- [ ] [LC-119-pascals-triangle-ii](https://leetcode.com/problems/pascals-triangle-ii)
- [ ] [LC-228-summary-ranges](https://leetcode.com/problems/summary-ranges)
- [ ] [LC-349-intersection-of-two-arrays](https://leetcode.com/problems/intersection-of-two-arrays)

## Day 01 - Arrays Basics

- [x] [LC-27-remove-element](https://leetcode.com/problems/remove-element)
- [ ] [LC-26-remove-duplicates-from-sorted-array](https://leetcode.com/problems/remove-duplicates-from-sorted-array)
- [ ] [LC-88-merge-sorted-array](https://leetcode.com/problems/merge-sorted-array)
- [ ] [LC-169-majority-element](https://leetcode.com/problems/majority-element)
- [ ] [LC-189-rotate-array](https://leetcode.com/problems/rotate-array)
- [ ] [LC-414-third-maximum-number](https://leetcode.com/problems/third-maximum-number)
- [ ] [LC-448-find-all-numbers-disappeared-in-an-array](https://leetcode.com/problems/find-all-numbers-disappeared-in-an-array)
- [ ] [LC-566-reshape-the-matrix](https://leetcode.com/problems/reshape-the-matrix)
- [ ] [LC-724-find-pivot-index](https://leetcode.com/problems/find-pivot-index)
- [ ] [LC-905-sort-array-by-parity](https://leetcode.com/problems/sort-array-by-parity)

## Day 02 - Strings and Two Pointers

- [ ] [LC-125-valid-palindrome](https://leetcode.com/problems/valid-palindrome)
- [ ] [LC-344-reverse-string](https://leetcode.com/problems/reverse-string)
- [ ] [LC-345-reverse-vowels-of-a-string](https://leetcode.com/problems/reverse-vowels-of-a-string)
- [ ] [LC-680-valid-palindrome-ii](https://leetcode.com/problems/valid-palindrome-ii)
- [ ] [LC-392-is-subsequence](https://leetcode.com/problems/is-subsequence)
- [ ] [LC-28-find-the-index-of-the-first-occurrence-in-a-string](https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string)
- [ ] [LC-415-add-strings](https://leetcode.com/problems/add-strings)
- [ ] [LC-58-length-of-last-word](https://leetcode.com/problems/length-of-last-word)
- [ ] [LC-557-reverse-words-in-a-string-iii](https://leetcode.com/problems/reverse-words-in-a-string-iii)
- [ ] [LC-796-rotate-string](https://leetcode.com/problems/rotate-string)

## Day 03 - Hashing and Frequency

- [ ] [LC-36-valid-sudoku](https://leetcode.com/problems/valid-sudoku)
- [ ] [LC-49-group-anagrams](https://leetcode.com/problems/group-anagrams)
- [ ] [LC-128-longest-consecutive-sequence](https://leetcode.com/problems/longest-consecutive-sequence)
- [ ] [LC-205-isomorphic-strings](https://leetcode.com/problems/isomorphic-strings)
- [ ] [LC-290-word-pattern](https://leetcode.com/problems/word-pattern)
- [ ] [LC-387-first-unique-character-in-a-string](https://leetcode.com/problems/first-unique-character-in-a-string)
- [ ] [LC-219-contains-duplicate-ii](https://leetcode.com/problems/contains-duplicate-ii)
- [ ] [LC-451-sort-characters-by-frequency](https://leetcode.com/problems/sort-characters-by-frequency)
- [ ] [LC-1207-unique-number-of-occurrences](https://leetcode.com/problems/unique-number-of-occurrences)
- [ ] [LC-347-top-k-frequent-elements](https://leetcode.com/problems/top-k-frequent-elements)

## Day 04 - Sliding Window and Prefix Sums

- [ ] [LC-3-longest-substring-without-repeating-characters](https://leetcode.com/problems/longest-substring-without-repeating-characters)
- [ ] [LC-209-minimum-size-subarray-sum](https://leetcode.com/problems/minimum-size-subarray-sum)
- [ ] [LC-239-sliding-window-maximum](https://leetcode.com/problems/sliding-window-maximum)
- [ ] [LC-567-permutation-in-string](https://leetcode.com/problems/permutation-in-string)
- [ ] [LC-643-maximum-average-subarray-i](https://leetcode.com/problems/maximum-average-subarray-i)
- [ ] [LC-930-binary-subarrays-with-sum](https://leetcode.com/problems/binary-subarrays-with-sum)
- [ ] [LC-560-subarray-sum-equals-k](https://leetcode.com/problems/subarray-sum-equals-k)
- [ ] [LC-974-subarray-sums-divisible-by-k](https://leetcode.com/problems/subarray-sums-divisible-by-k)
- [ ] [LC-713-subarray-product-less-than-k](https://leetcode.com/problems/subarray-product-less-than-k)
- [ ] [LC-424-longest-repeating-character-replacement](https://leetcode.com/problems/longest-repeating-character-replacement)

## Day 05 - Linked Lists

- [ ] [LC-206-reverse-linked-list](https://leetcode.com/problems/reverse-linked-list)
- [ ] [LC-83-remove-duplicates-from-sorted-list](https://leetcode.com/problems/remove-duplicates-from-sorted-list)
- [ ] [LC-141-linked-list-cycle](https://leetcode.com/problems/linked-list-cycle)
- [ ] [LC-142-linked-list-cycle-ii](https://leetcode.com/problems/linked-list-cycle-ii)
- [ ] [LC-19-remove-nth-node-from-end-of-list](https://leetcode.com/problems/remove-nth-node-from-end-of-list)
- [ ] [LC-160-intersection-of-two-linked-lists](https://leetcode.com/problems/intersection-of-two-linked-lists)
- [ ] [LC-234-palindrome-linked-list](https://leetcode.com/problems/palindrome-linked-list)
- [ ] [LC-86-partition-list](https://leetcode.com/problems/partition-list)
- [ ] [LC-61-rotate-list](https://leetcode.com/problems/rotate-list)
- [ ] [LC-92-reverse-linked-list-ii](https://leetcode.com/problems/reverse-linked-list-ii)

## Day 06 - Stacks and Queues

- [ ] [LC-150-evaluate-reverse-polish-notation](https://leetcode.com/problems/evaluate-reverse-polish-notation)
- [ ] [LC-71-simplify-path](https://leetcode.com/problems/simplify-path)
- [ ] [LC-232-implement-queue-using-stacks](https://leetcode.com/problems/implement-queue-using-stacks)
- [ ] [LC-225-implement-stack-using-queues](https://leetcode.com/problems/implement-stack-using-queues)
- [ ] [LC-496-next-greater-element-i](https://leetcode.com/problems/next-greater-element-i)
- [ ] [LC-503-next-greater-element-ii](https://leetcode.com/problems/next-greater-element-ii)
- [ ] [LC-739-daily-temperatures](https://leetcode.com/problems/daily-temperatures)
- [ ] [LC-901-online-stock-span](https://leetcode.com/problems/online-stock-span)
- [ ] [LC-622-design-circular-queue](https://leetcode.com/problems/design-circular-queue)
- [ ] [LC-933-number-of-recent-calls](https://leetcode.com/problems/number-of-recent-calls)

## Day 07 - Binary Search and Sorting

- [ ] [LC-704-binary-search](https://leetcode.com/problems/binary-search)
- [ ] [LC-34-find-first-and-last-position-of-element-in-sorted-array](https://leetcode.com/problems/find-first-and-last-position-of-element-in-sorted-array)
- [ ] [LC-33-search-in-rotated-sorted-array](https://leetcode.com/problems/search-in-rotated-sorted-array)
- [ ] [LC-74-search-a-2d-matrix](https://leetcode.com/problems/search-a-2d-matrix)
- [ ] [LC-153-find-minimum-in-rotated-sorted-array](https://leetcode.com/problems/find-minimum-in-rotated-sorted-array)
- [ ] [LC-162-find-peak-element](https://leetcode.com/problems/find-peak-element)
- [ ] [LC-69-sqrtx](https://leetcode.com/problems/sqrtx)
- [ ] [LC-278-first-bad-version](https://leetcode.com/problems/first-bad-version)
- [ ] [LC-875-koko-eating-bananas](https://leetcode.com/problems/koko-eating-bananas)
- [ ] [LC-658-find-k-closest-elements](https://leetcode.com/problems/find-k-closest-elements)

## Day 08 - Intervals and Greedy

- [ ] [LC-56-merge-intervals](https://leetcode.com/problems/merge-intervals)
- [ ] [LC-57-insert-interval](https://leetcode.com/problems/insert-interval)
- [ ] [LC-435-non-overlapping-intervals](https://leetcode.com/problems/non-overlapping-intervals)
- [ ] [LC-452-minimum-number-of-arrows-to-burst-balloons](https://leetcode.com/problems/minimum-number-of-arrows-to-burst-balloons)
- [ ] [LC-122-best-time-to-buy-and-sell-stock-ii](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-ii)
- [ ] [LC-55-jump-game](https://leetcode.com/problems/jump-game)
- [ ] [LC-45-jump-game-ii](https://leetcode.com/problems/jump-game-ii)
- [ ] [LC-134-gas-station](https://leetcode.com/problems/gas-station)
- [ ] [LC-763-partition-labels](https://leetcode.com/problems/partition-labels)
- [ ] [LC-406-queue-reconstruction-by-height](https://leetcode.com/problems/queue-reconstruction-by-height)

## Day 09 - Binary Trees I

- [ ] [LC-94-binary-tree-inorder-traversal](https://leetcode.com/problems/binary-tree-inorder-traversal)
- [ ] [LC-144-binary-tree-preorder-traversal](https://leetcode.com/problems/binary-tree-preorder-traversal)
- [ ] [LC-145-binary-tree-postorder-traversal](https://leetcode.com/problems/binary-tree-postorder-traversal)
- [ ] [LC-102-binary-tree-level-order-traversal](https://leetcode.com/problems/binary-tree-level-order-traversal)
- [ ] [LC-101-symmetric-tree](https://leetcode.com/problems/symmetric-tree)
- [ ] [LC-100-same-tree](https://leetcode.com/problems/same-tree)
- [ ] [LC-110-balanced-binary-tree](https://leetcode.com/problems/balanced-binary-tree)
- [ ] [LC-111-minimum-depth-of-binary-tree](https://leetcode.com/problems/minimum-depth-of-binary-tree)
- [ ] [LC-112-path-sum](https://leetcode.com/problems/path-sum)
- [ ] [LC-257-binary-tree-paths](https://leetcode.com/problems/binary-tree-paths)

## Day 10 - BST and Tree Utilities

- [ ] [LC-700-search-in-a-binary-search-tree](https://leetcode.com/problems/search-in-a-binary-search-tree)
- [ ] [LC-701-insert-into-a-binary-search-tree](https://leetcode.com/problems/insert-into-a-binary-search-tree)
- [ ] [LC-98-validate-binary-search-tree](https://leetcode.com/problems/validate-binary-search-tree)
- [ ] [LC-235-lowest-common-ancestor-of-a-binary-search-tree](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree)
- [ ] [LC-450-delete-node-in-a-bst](https://leetcode.com/problems/delete-node-in-a-bst)
- [ ] [LC-530-minimum-absolute-difference-in-bst](https://leetcode.com/problems/minimum-absolute-difference-in-bst)
- [ ] [LC-108-convert-sorted-array-to-binary-search-tree](https://leetcode.com/problems/convert-sorted-array-to-binary-search-tree)
- [ ] [LC-109-convert-sorted-list-to-binary-search-tree](https://leetcode.com/problems/convert-sorted-list-to-binary-search-tree)
- [ ] [LC-938-range-sum-of-bst](https://leetcode.com/problems/range-sum-of-bst)
- [ ] [LC-173-binary-search-tree-iterator](https://leetcode.com/problems/binary-search-tree-iterator)

## Day 11 - Heaps and Priority Queues

- [ ] [LC-1046-last-stone-weight](https://leetcode.com/problems/last-stone-weight)
- [ ] [LC-215-kth-largest-element-in-an-array](https://leetcode.com/problems/kth-largest-element-in-an-array)
- [ ] [LC-692-top-k-frequent-words](https://leetcode.com/problems/top-k-frequent-words)
- [ ] [LC-973-k-closest-points-to-origin](https://leetcode.com/problems/k-closest-points-to-origin)
- [ ] [LC-373-find-k-pairs-with-smallest-sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums)
- [ ] [LC-378-kth-smallest-element-in-a-sorted-matrix](https://leetcode.com/problems/kth-smallest-element-in-a-sorted-matrix)
- [ ] [LC-264-ugly-number-ii](https://leetcode.com/problems/ugly-number-ii)
- [ ] [LC-767-reorganize-string](https://leetcode.com/problems/reorganize-string)
- [ ] [LC-621-task-scheduler](https://leetcode.com/problems/task-scheduler)
- [ ] [LC-502-ipo](https://leetcode.com/problems/ipo)

## Day 12 - Backtracking I

- [ ] [LC-17-letter-combinations-of-a-phone-number](https://leetcode.com/problems/letter-combinations-of-a-phone-number)
- [ ] [LC-22-generate-parentheses](https://leetcode.com/problems/generate-parentheses)
- [ ] [LC-39-combination-sum](https://leetcode.com/problems/combination-sum)
- [ ] [LC-40-combination-sum-ii](https://leetcode.com/problems/combination-sum-ii)
- [ ] [LC-46-permutations](https://leetcode.com/problems/permutations)
- [ ] [LC-47-permutations-ii](https://leetcode.com/problems/permutations-ii)
- [ ] [LC-77-combinations](https://leetcode.com/problems/combinations)
- [ ] [LC-78-subsets](https://leetcode.com/problems/subsets)
- [ ] [LC-90-subsets-ii](https://leetcode.com/problems/subsets-ii)
- [ ] [LC-131-palindrome-partitioning](https://leetcode.com/problems/palindrome-partitioning)

## Day 13 - Backtracking II

- [ ] [LC-93-restore-ip-addresses](https://leetcode.com/problems/restore-ip-addresses)
- [ ] [LC-79-word-search](https://leetcode.com/problems/word-search)
- [ ] [LC-216-combination-sum-iii](https://leetcode.com/problems/combination-sum-iii)
- [ ] [LC-491-increasing-subsequences](https://leetcode.com/problems/increasing-subsequences)
- [ ] [LC-784-letter-case-permutation](https://leetcode.com/problems/letter-case-permutation)
- [ ] [LC-306-additive-number](https://leetcode.com/problems/additive-number)
- [ ] [LC-254-factor-combinations](https://leetcode.com/problems/factor-combinations)
- [ ] [LC-386-lexicographical-numbers](https://leetcode.com/problems/lexicographical-numbers)
- [ ] [LC-1079-letter-tile-possibilities](https://leetcode.com/problems/letter-tile-possibilities)
- [ ] [LC-89-gray-code](https://leetcode.com/problems/gray-code)

## Day 14 - Graphs I

- [ ] [LC-200-number-of-islands](https://leetcode.com/problems/number-of-islands)
- [ ] [LC-695-max-area-of-island](https://leetcode.com/problems/max-area-of-island)
- [ ] [LC-733-flood-fill](https://leetcode.com/problems/flood-fill)
- [ ] [LC-994-rotting-oranges](https://leetcode.com/problems/rotting-oranges)
- [ ] [LC-130-surrounded-regions](https://leetcode.com/problems/surrounded-regions)
- [ ] [LC-133-clone-graph](https://leetcode.com/problems/clone-graph)
- [ ] [LC-207-course-schedule](https://leetcode.com/problems/course-schedule)
- [ ] [LC-210-course-schedule-ii](https://leetcode.com/problems/course-schedule-ii)
- [ ] [LC-547-number-of-provinces](https://leetcode.com/problems/number-of-provinces)
- [ ] [LC-785-is-graph-bipartite](https://leetcode.com/problems/is-graph-bipartite)

## Day 15 - Union-Find and Graph Connectivity

- [ ] [LC-261-graph-valid-tree](https://leetcode.com/problems/graph-valid-tree)
- [ ] [LC-323-number-of-connected-components-in-an-undirected-graph](https://leetcode.com/problems/number-of-connected-components-in-an-undirected-graph)
- [ ] [LC-399-evaluate-division](https://leetcode.com/problems/evaluate-division)
- [ ] [LC-684-redundant-connection](https://leetcode.com/problems/redundant-connection)
- [ ] [LC-721-accounts-merge](https://leetcode.com/problems/accounts-merge)
- [ ] [LC-947-most-stones-removed-with-same-row-or-column](https://leetcode.com/problems/most-stones-removed-with-same-row-or-column)
- [ ] [LC-990-satisfiability-of-equality-equations](https://leetcode.com/problems/satisfiability-of-equality-equations)
- [ ] [LC-1061-lexicographically-smallest-equivalent-string](https://leetcode.com/problems/lexicographically-smallest-equivalent-string)
- [ ] [LC-1319-number-of-operations-to-make-network-connected](https://leetcode.com/problems/number-of-operations-to-make-network-connected)
- [ ] [LC-1135-connecting-cities-with-minimum-cost](https://leetcode.com/problems/connecting-cities-with-minimum-cost)

## Day 16 - Bit Manipulation

- [ ] [LC-136-single-number](https://leetcode.com/problems/single-number)
- [ ] [LC-137-single-number-ii](https://leetcode.com/problems/single-number-ii)
- [ ] [LC-190-reverse-bits](https://leetcode.com/problems/reverse-bits)
- [ ] [LC-191-number-of-1-bits](https://leetcode.com/problems/number-of-1-bits)
- [ ] [LC-231-power-of-two](https://leetcode.com/problems/power-of-two)
- [ ] [LC-268-missing-number](https://leetcode.com/problems/missing-number)
- [ ] [LC-201-bitwise-and-of-numbers-range](https://leetcode.com/problems/bitwise-and-of-numbers-range)
- [ ] [LC-338-counting-bits](https://leetcode.com/problems/counting-bits)
- [ ] [LC-461-hamming-distance](https://leetcode.com/problems/hamming-distance)
- [ ] [LC-476-number-complement](https://leetcode.com/problems/number-complement)

## Day 17 - Dynamic Programming I

- [ ] [LC-70-climbing-stairs](https://leetcode.com/problems/climbing-stairs)
- [ ] [LC-198-house-robber](https://leetcode.com/problems/house-robber)
- [ ] [LC-213-house-robber-ii](https://leetcode.com/problems/house-robber-ii)
- [ ] [LC-509-fibonacci-number](https://leetcode.com/problems/fibonacci-number)
- [ ] [LC-746-min-cost-climbing-stairs](https://leetcode.com/problems/min-cost-climbing-stairs)
- [ ] [LC-62-unique-paths](https://leetcode.com/problems/unique-paths)
- [ ] [LC-63-unique-paths-ii](https://leetcode.com/problems/unique-paths-ii)
- [ ] [LC-64-minimum-path-sum](https://leetcode.com/problems/minimum-path-sum)
- [ ] [LC-120-triangle](https://leetcode.com/problems/triangle)
- [ ] [LC-91-decode-ways](https://leetcode.com/problems/decode-ways)

## Day 18 - Dynamic Programming II

- [ ] [LC-300-longest-increasing-subsequence](https://leetcode.com/problems/longest-increasing-subsequence)
- [ ] [LC-53-maximum-subarray](https://leetcode.com/problems/maximum-subarray)
- [ ] [LC-152-maximum-product-subarray](https://leetcode.com/problems/maximum-product-subarray)
- [ ] [LC-139-word-break](https://leetcode.com/problems/word-break)
- [ ] [LC-416-partition-equal-subset-sum](https://leetcode.com/problems/partition-equal-subset-sum)
- [ ] [LC-494-target-sum](https://leetcode.com/problems/target-sum)
- [ ] [LC-377-combination-sum-iv](https://leetcode.com/problems/combination-sum-iv)
- [ ] [LC-647-palindromic-substrings](https://leetcode.com/problems/palindromic-substrings)
- [ ] [LC-5-longest-palindromic-substring](https://leetcode.com/problems/longest-palindromic-substring)
- [ ] [LC-673-number-of-longest-increasing-subsequence](https://leetcode.com/problems/number-of-longest-increasing-subsequence)

## Day 19 - Dynamic Programming III

- [ ] [LC-1143-longest-common-subsequence](https://leetcode.com/problems/longest-common-subsequence)
- [ ] [LC-583-delete-operation-for-two-strings](https://leetcode.com/problems/delete-operation-for-two-strings)
- [ ] [LC-97-interleaving-string](https://leetcode.com/problems/interleaving-string)
- [ ] [LC-516-longest-palindromic-subsequence](https://leetcode.com/problems/longest-palindromic-subsequence)
- [ ] [LC-221-maximal-square](https://leetcode.com/problems/maximal-square)
- [ ] [LC-279-perfect-squares](https://leetcode.com/problems/perfect-squares)
- [ ] [LC-322-coin-change](https://leetcode.com/problems/coin-change)
- [ ] [LC-518-coin-change-ii](https://leetcode.com/problems/coin-change-ii)
- [ ] [LC-368-largest-divisible-subset](https://leetcode.com/problems/largest-divisible-subset)
- [ ] [LC-1035-uncrossed-lines](https://leetcode.com/problems/uncrossed-lines)

## Day 20 - Tree Traversals and BFS

- [ ] [LC-103-binary-tree-zigzag-level-order-traversal](https://leetcode.com/problems/binary-tree-zigzag-level-order-traversal)
- [ ] [LC-199-binary-tree-right-side-view](https://leetcode.com/problems/binary-tree-right-side-view)
- [ ] [LC-515-find-largest-value-in-each-tree-row](https://leetcode.com/problems/find-largest-value-in-each-tree-row)
- [ ] [LC-637-average-of-levels-in-binary-tree](https://leetcode.com/problems/average-of-levels-in-binary-tree)
- [ ] [LC-222-count-complete-tree-nodes](https://leetcode.com/problems/count-complete-tree-nodes)
- [ ] [LC-226-invert-binary-tree](https://leetcode.com/problems/invert-binary-tree)
- [ ] [LC-230-kth-smallest-element-in-a-bst](https://leetcode.com/problems/kth-smallest-element-in-a-bst)
- [ ] [LC-236-lowest-common-ancestor-of-a-binary-tree](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-tree)
- [ ] [LC-572-subtree-of-another-tree](https://leetcode.com/problems/subtree-of-another-tree)
- [ ] [LC-654-maximum-binary-tree](https://leetcode.com/problems/maximum-binary-tree)

## Day 21 - Tree Construction and Recursion

- [ ] [LC-105-construct-binary-tree-from-preorder-and-inorder-traversal](https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal)
- [ ] [LC-106-construct-binary-tree-from-inorder-and-postorder-traversal](https://leetcode.com/problems/construct-binary-tree-from-inorder-and-postorder-traversal)
- [ ] [LC-114-flatten-binary-tree-to-linked-list](https://leetcode.com/problems/flatten-binary-tree-to-linked-list)
- [ ] [LC-116-populating-next-right-pointers-in-each-node](https://leetcode.com/problems/populating-next-right-pointers-in-each-node)
- [ ] [LC-117-populating-next-right-pointers-in-each-node-ii](https://leetcode.com/problems/populating-next-right-pointers-in-each-node-ii)
- [ ] [LC-129-sum-root-to-leaf-numbers](https://leetcode.com/problems/sum-root-to-leaf-numbers)
- [ ] [LC-337-house-robber-iii](https://leetcode.com/problems/house-robber-iii)
- [ ] [LC-437-path-sum-iii](https://leetcode.com/problems/path-sum-iii)
- [ ] [LC-623-add-one-row-to-tree](https://leetcode.com/problems/add-one-row-to-tree)
- [ ] [LC-653-two-sum-iv-input-is-a-bst](https://leetcode.com/problems/two-sum-iv-input-is-a-bst)

## Day 22 - Heaps and Scheduling

- [ ] [LC-1046-last-stone-weight](https://leetcode.com/problems/last-stone-weight)
- [ ] [LC-215-kth-largest-element-in-an-array](https://leetcode.com/problems/kth-largest-element-in-an-array)
- [ ] [LC-692-top-k-frequent-words](https://leetcode.com/problems/top-k-frequent-words)
- [ ] [LC-973-k-closest-points-to-origin](https://leetcode.com/problems/k-closest-points-to-origin)
- [ ] [LC-373-find-k-pairs-with-smallest-sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums)
- [ ] [LC-378-kth-smallest-element-in-a-sorted-matrix](https://leetcode.com/problems/kth-smallest-element-in-a-sorted-matrix)
- [ ] [LC-264-ugly-number-ii](https://leetcode.com/problems/ugly-number-ii)
- [ ] [LC-767-reorganize-string](https://leetcode.com/problems/reorganize-string)
- [ ] [LC-621-task-scheduler](https://leetcode.com/problems/task-scheduler)
- [ ] [LC-502-ipo](https://leetcode.com/problems/ipo)

## Day 23 - Matrix Problems

- [ ] [LC-54-spiral-matrix](https://leetcode.com/problems/spiral-matrix)
- [ ] [LC-59-spiral-matrix-ii](https://leetcode.com/problems/spiral-matrix-ii)
- [ ] [LC-73-set-matrix-zeroes](https://leetcode.com/problems/set-matrix-zeroes)
- [ ] [LC-48-rotate-image](https://leetcode.com/problems/rotate-image)
- [ ] [LC-867-transpose-matrix](https://leetcode.com/problems/transpose-matrix)
- [ ] [LC-498-diagonal-traverse](https://leetcode.com/problems/diagonal-traverse)
- [ ] [LC-1572-matrix-diagonal-sum](https://leetcode.com/problems/matrix-diagonal-sum)
- [ ] [LC-240-search-a-2d-matrix-ii](https://leetcode.com/problems/search-a-2d-matrix-ii)
- [ ] [LC-885-spiral-matrix-iii](https://leetcode.com/problems/spiral-matrix-iii)
- [ ] [LC-1260-shift-2d-grid](https://leetcode.com/problems/shift-2d-grid)

## Day 24 - String Parsing and Stacks

- [ ] [LC-151-reverse-words-in-a-string](https://leetcode.com/problems/reverse-words-in-a-string)
- [ ] [LC-186-reverse-words-in-a-string-ii](https://leetcode.com/problems/reverse-words-in-a-string-ii)
- [ ] [LC-168-excel-sheet-column-title](https://leetcode.com/problems/excel-sheet-column-title)
- [ ] [LC-171-excel-sheet-column-number](https://leetcode.com/problems/excel-sheet-column-number)
- [ ] [LC-443-string-compression](https://leetcode.com/problems/string-compression)
- [ ] [LC-541-reverse-string-ii](https://leetcode.com/problems/reverse-string-ii)
- [ ] [LC-844-backspace-string-compare](https://leetcode.com/problems/backspace-string-compare)
- [ ] [LC-394-decode-string](https://leetcode.com/problems/decode-string)
- [ ] [LC-227-basic-calculator-ii](https://leetcode.com/problems/basic-calculator-ii)
- [ ] [LC-921-minimum-add-to-make-parentheses-valid](https://leetcode.com/problems/minimum-add-to-make-parentheses-valid)

## Day 25 - Prefix Sums and Difference Arrays

- [ ] [LC-303-range-sum-query-immutable](https://leetcode.com/problems/range-sum-query-immutable)
- [ ] [LC-304-range-sum-query-2d-immutable](https://leetcode.com/problems/range-sum-query-2d-immutable)
- [ ] [LC-413-arithmetic-slices](https://leetcode.com/problems/arithmetic-slices)
- [ ] [LC-525-contiguous-array](https://leetcode.com/problems/contiguous-array)
- [ ] [LC-985-sum-of-even-numbers-after-queries](https://leetcode.com/problems/sum-of-even-numbers-after-queries)
- [ ] [LC-1109-corporate-flight-bookings](https://leetcode.com/problems/corporate-flight-bookings)
- [ ] [LC-1094-car-pooling](https://leetcode.com/problems/car-pooling)
- [ ] [LC-1310-xor-queries-of-a-subarray](https://leetcode.com/problems/xor-queries-of-a-subarray)
- [ ] [LC-1480-running-sum-of-1d-array](https://leetcode.com/problems/running-sum-of-1d-array)
- [ ] [LC-1590-make-sum-divisible-by-p](https://leetcode.com/problems/make-sum-divisible-by-p)

## Day 26 - Arrays and Greedy Review

- [ ] [LC-41-first-missing-positive](https://leetcode.com/problems/first-missing-positive)
- [ ] [LC-66-plus-one](https://leetcode.com/problems/plus-one)
- [ ] [LC-67-add-binary](https://leetcode.com/problems/add-binary)
- [ ] [LC-75-sort-colors](https://leetcode.com/problems/sort-colors)
- [ ] [LC-80-remove-duplicates-from-sorted-array-ii](https://leetcode.com/problems/remove-duplicates-from-sorted-array-ii)
- [ ] [LC-179-largest-number](https://leetcode.com/problems/largest-number)
- [ ] [LC-238-product-of-array-except-self](https://leetcode.com/problems/product-of-array-except-self)
- [ ] [LC-324-wiggle-sort-ii](https://leetcode.com/problems/wiggle-sort-ii)
- [ ] [LC-769-max-chunks-to-make-sorted](https://leetcode.com/problems/max-chunks-to-make-sorted)
- [ ] [LC-977-squares-of-a-sorted-array](https://leetcode.com/problems/squares-of-a-sorted-array)

## Day 27 - Linked Lists II

- [ ] [LC-2-add-two-numbers](https://leetcode.com/problems/add-two-numbers)
- [ ] [LC-24-swap-nodes-in-pairs](https://leetcode.com/problems/swap-nodes-in-pairs)
- [ ] [LC-25-reverse-nodes-in-k-group](https://leetcode.com/problems/reverse-nodes-in-k-group)
- [ ] [LC-82-remove-duplicates-from-sorted-list-ii](https://leetcode.com/problems/remove-duplicates-from-sorted-list-ii)
- [ ] [LC-138-copy-list-with-random-pointer](https://leetcode.com/problems/copy-list-with-random-pointer)
- [ ] [LC-143-reorder-list](https://leetcode.com/problems/reorder-list)
- [ ] [LC-148-sort-list](https://leetcode.com/problems/sort-list)
- [ ] [LC-203-remove-linked-list-elements](https://leetcode.com/problems/remove-linked-list-elements)
- [ ] [LC-328-odd-even-linked-list](https://leetcode.com/problems/odd-even-linked-list)
- [ ] [LC-445-add-two-numbers-ii](https://leetcode.com/problems/add-two-numbers-ii)

## Day 28 - Graphs Shortest Paths

- [ ] [LC-743-network-delay-time](https://leetcode.com/problems/network-delay-time)
- [ ] [LC-787-cheapest-flights-within-k-stops](https://leetcode.com/problems/cheapest-flights-within-k-stops)
- [ ] [LC-797-all-paths-from-source-to-target](https://leetcode.com/problems/all-paths-from-source-to-target)
- [ ] [LC-802-find-eventual-safe-states](https://leetcode.com/problems/find-eventual-safe-states)
- [ ] [LC-841-keys-and-rooms](https://leetcode.com/problems/keys-and-rooms)
- [ ] [LC-886-possible-bipartition](https://leetcode.com/problems/possible-bipartition)
- [ ] [LC-1091-shortest-path-in-binary-matrix](https://leetcode.com/problems/shortest-path-in-binary-matrix)
- [ ] [LC-1197-minimum-knight-moves](https://leetcode.com/problems/minimum-knight-moves)
- [ ] [LC-1334-find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance](https://leetcode.com/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance)
- [ ] [LC-1514-path-with-maximum-probability](https://leetcode.com/problems/path-with-maximum-probability)

## Day 29 - Design Problems

- [ ] [LC-208-implement-trie-prefix-tree](https://leetcode.com/problems/implement-trie-prefix-tree)
- [ ] [LC-211-design-add-and-search-words-data-structure](https://leetcode.com/problems/design-add-and-search-words-data-structure)
- [ ] [LC-380-insert-delete-getrandom-o1](https://leetcode.com/problems/insert-delete-getrandom-o1)
- [ ] [LC-705-design-hashset](https://leetcode.com/problems/design-hashset)
- [ ] [LC-706-design-hashmap](https://leetcode.com/problems/design-hashmap)
- [ ] [LC-146-lru-cache](https://leetcode.com/problems/lru-cache)
- [ ] [LC-981-time-based-key-value-store](https://leetcode.com/problems/time-based-key-value-store)
- [ ] [LC-348-design-tic-tac-toe](https://leetcode.com/problems/design-tic-tac-toe)
- [ ] [LC-729-my-calendar-i](https://leetcode.com/problems/my-calendar-i)
- [ ] [LC-641-design-circular-deque](https://leetcode.com/problems/design-circular-deque)

## Day 30 - Mixed Review

- [ ] [LC-31-next-permutation](https://leetcode.com/problems/next-permutation)
- [ ] [LC-43-multiply-strings](https://leetcode.com/problems/multiply-strings)
- [ ] [LC-50-powx-n](https://leetcode.com/problems/powx-n)
- [ ] [LC-118-pascals-triangle](https://leetcode.com/problems/pascals-triangle)
- [ ] [LC-119-pascals-triangle-ii](https://leetcode.com/problems/pascals-triangle-ii)
- [ ] [LC-228-summary-ranges](https://leetcode.com/problems/summary-ranges)
- [ ] [LC-349-intersection-of-two-arrays](https://leetcode.com/problems/intersection-of-two-arrays)
- [ ] [LC-350-intersection-of-two-arrays-ii](https://leetcode.com/problems/intersection-of-two-arrays-ii)
- [ ] [LC-376-wiggle-subsequence](https://leetcode.com/problems/wiggle-subsequence)
- [ ] [LC-650-2-keys-keyboard](https://leetcode.com/problems/2-keys-keyboard)

## Solved Problems In Folder

- [x] [LC-1-two-sum](https://leetcode.com/problems/two-sum)
- [x] [LC-9-palindrome-number](https://leetcode.com/problems/palindrome-number)
- [x] [LC-12-integer-to-roman](https://leetcode.com/problems/integer-to-roman)
- [x] [LC-13-roman-to-integer](https://leetcode.com/problems/roman-to-integer)
- [x] [LC-14-longest-common-prefix](https://leetcode.com/problems/longest-common-prefix)
- [x] [LC-20-valid-parentheses](https://leetcode.com/problems/valid-parentheses)
- [x] [LC-21-merge-two-sorted-lists](https://leetcode.com/problems/merge-two-sorted-lists)
- [x] [LC-104-maximum-depth-of-binary-tree](https://leetcode.com/problems/maximum-depth-of-binary-tree)
- [x] [LC-121-best-time-to-buy-and-sell-stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock)
- [x] [LC-155-min-stack](https://leetcode.com/problems/min-stack)
- [x] [LC-217-contains-duplicate](https://leetcode.com/problems/contains-duplicate)
- [x] [LC-242-valid-anagram](https://leetcode.com/problems/valid-anagram)
- [x] [LC-283-move-zeroes](https://leetcode.com/problems/move-zeroes)
- [x] [LC-292-nim-game](https://leetcode.com/problems/nim-game)
- [x] [LC-412-fizz-buzz](https://leetcode.com/problems/fizz-buzz)
- [x] [LC-485-max-consecutive-ones](https://leetcode.com/problems/max-consecutive-ones)
- [x] [LC-1015-smallest-integer-divisible-by-k](https://leetcode.com/problems/smallest-integer-divisible-by-k)
- [x] [LC-1018-binary-prefix-divisible-by-5](https://leetcode.com/problems/binary-prefix-divisible-by-5)
- [x] [LC-1262-greatest-sum-divisible-by-three](https://leetcode.com/problems/greatest-sum-divisible-by-three)
- [x] [LC-1470-shuffle-the-array](https://leetcode.com/problems/shuffle-the-array)
- [x] [LC-1929-concatenation-of-array](https://leetcode.com/problems/concatenation-of-array)
- [x] [LC-3381-maximum-subarray-sum-with-length-divisible-by-k](https://leetcode.com/problems/maximum-subarray-sum-with-length-divisible-by-k)
